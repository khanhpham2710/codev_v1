...
public class Pagination extends JPanel {
    ...
    private List<PaginationEvent> events = new ArrayList<>();
    private Page page;
    ...
    private Page paginate(int current, int max) {
        ...
        int r2 = current + r;
        if (current > 4) {
            items.add(new Page.BreakLabel((Math.max(r1, 2)) - 1));
        }
        for (int i = Math.max(r1, 2); i <= Math.min(max, r2); i++) {
            items.add(i);
        }
        if (r2 + 1 < max) {
            items.add(new Page.BreakLabel(Integer.parseInt(items.getLast().toString()) + 1));
        }
        if (r2 < max) {
            items.add(max);
        }
